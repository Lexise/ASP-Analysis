...
def global_store():
    ...
    if len(files)>1:
        ...
        print("start process")
        processed_data, bar_data, correlation_matrix,cluster_feature,group_feature = process_data(UPLOAD_DIRECTORY+question, UPLOAD_DIRECTORY+answer)
        group_feature.to_pickle(PROCESSED_DIRECTORY + "group_feature.pkl")
        cluster_feature.to_pickle(PROCESSED_DIRECTORY + "cluster_feature.pkl")
...
@app.callback([Output('scatter_cluster', 'figure'), Output('scatter_groups', 'figure'), Output('table1', 'children'),Output('table2', 'children')],
              [ Input('signal', 'figure')])
@cache.memoize()
def generate_tabs( content ):#processed_data, table1_data,table2_data ):
    if  content==False:#load and processed
        ...
        cluster_table = pd.read_pickle(PROCESSED_DIRECTORY + "cluster_feature.pkl")
    else:
        if content=="": #have not load
            processed_data=df
            group_table = report_groups
            cluster_table = report_cluster
        else:
    ...
    figure1 = {
        'data': [
            {
                'x': processed_data[processed_data.cluster_label == cls].position_x,
                'y': processed_data[processed_data.cluster_label == cls].position_y,
    ...
    figure2 = {
        'data': [
            {
                'x': processed_data[processed_data.groups == cls].position_x,
                'y': processed_data[processed_data.groups == cls].position_y,
    ...
    if len(group_table)==0:
        table1=html.H1("No group feature")
    else:
